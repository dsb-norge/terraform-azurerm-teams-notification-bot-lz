...
run "apply" {
  ...
  # (`log-<name>`) — proves the module didn't create its own workspace.
  assert {
    condition     = !can(regex("/log-${run.setup.name}$", output.log_analytics_workspace_id))

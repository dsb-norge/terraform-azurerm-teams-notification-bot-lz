...
| <a name="input_debug_ip_rules"></a> [debug\_ip\_rules](#input\_debug\_ip\_rules) | CIDR ranges allowed inbound for human operators and CI runners. Applied to:<br/>  - the function app's public endpoint (manual /api/* smoke-testing)<br/>  - the SCM/Kudu endpoint (live log streaming, portal debugging)<br/>  - the storage account network rules (terraform apply uploading deployment packages)<br/><br/>This is NOT for application callers pushing messages to the API — use<br/>allowed\_caller\_rules for that. Typical use: the DSB VPN CIDR for operators. | <pre>list(object({<br/>    name        = string<br/>    description = string<br/>    cidr        = string<br/>  }))</pre> | `[]` | no |
